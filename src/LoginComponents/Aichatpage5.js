...
export default function Aichatpage() {
  ...
  const Characters = "AK";
        ...
        async function handleSubmit(e){
          ...
          try{
            const response = await fetch("http://localhost:9001/instagram-captions",{
              method:'POST',
              headers:{
    ...
    <div style={{}} className='flex flex-row max-h-screen'>
      ...
      <div className='mt-6'>
            ...
            <div>
              ...
              <label className='text-white'>What is your post about?</label>
              <textarea value={input} onChange={handleChange1} className='rounded-lg' style={{padding:'0.7ch'}} placeholder='Finally we reach 1M followers...'></textarea>
              {showInput && 
              <div className='text-sm' style={{color:'red'}}>Your post must have at least 2 characters</div>
              }
              </div>
...
<Link id='automate' onClick={handleMenuToggle} className='bg-white flex-row'>
        <div style={{marginLeft:'3ch'}} className='flex flex-row space-x-2 items-center'>
          ...
          <span id="colors" style={{color:'#962fbf'}} className='items-center text-2xl'> <AiFillInstagram/> </span>
          </div>
         ...
         <span>
          <p>AI tool generates Instagram Captions on user-defined parameters.</p>
         </span>
          </div> 
        </div>
</Link>
