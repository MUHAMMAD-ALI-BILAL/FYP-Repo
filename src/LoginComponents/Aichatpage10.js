...
export default function Aichatpage() {
  ...
  const Characters = "AK";
        ...
        async function handleSubmit(e){
          ...
          try{
            const response = await fetch("http://localhost:9001/amazonproducttitle",{
              method:'POST',
              headers:{
  ...
  return (
    ...
    <div style={{}} className='flex flex-row max-h-screen'>
      ...
      <div>
              ...
              <input className='rounded-lg' style={{padding:'0.7ch'}} value={input} onChange={handleChange1} placeholder='Water bottle'></input>
              {showInput && 
              ...
              <textarea className='rounded-lg' style={{padding:'0.7ch'}} value={keyword} onChange={handleChange2} placeholder='Bottle has a height of around 8-10 inches and a diameter of 2-3 inches. Bottle is made from high-quality materials and is perfect for use'></textarea>
              {showKeyboard && 
              ...
              <textarea className='rounded-lg' style={{padding:'0.7ch'}} value={target} onChange={handleChange3} placeholder='water bottle, perfect water bottle, easy to carry'></textarea>
              {/* {showKeyboard && 
              <div className='text-sm' style={{color:'red'}}>Search Term must have at least 2 characters</div>
              }
               {showCharacter && 
...
<Link id='automate' onClick={handleMenuToggle} className='bg-white flex-row'>
        <div style={{marginLeft:'3ch'}} className='flex flex-row space-x-2 items-center'>
          ...
          <span id="colors" style={{color:'#FF9900'}} className='items-center text-2xl'> <ImAmazon/> </span>
          </div>

           <div className=''>
            <div className='flex flex-row'>
          <span className='text-2xl font-sm'>
          <p>Amzaon Product Title</p>
          </span>
         ...
         <span>
          <p>AI tool generates Amzaon Product Title on user-defined parameters.</p>
         </span>
          </div> 
        </div>
</Link>
