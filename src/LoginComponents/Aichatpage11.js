...
export default function Aichatpage() {
  ...
  const Characters = "AK";
        ...
        async function handleSubmit(e){
          ...
          try{
            const response = await fetch("http://localhost:9001/productdescription",{
              method:'POST',
              headers:{
  ...
  return (
    ...
    <div style={{}} className='flex flex-row max-h-screen'>
      ...
      <div>
              ...
              {showInput && 
              <div className='text-sm' style={{color:'red'}}>Product must have at least 2 characters</div>
              }
              </div>

              <div style={{width:'90%', marginLeft:'auto', marginRight:'auto'}} className='flex flex-col'>
              <label className='text-white'>Short Description</label>
              <textarea className='rounded-lg' style={{padding:'0.7ch'}} value={keyword} onChange={handleChange2} placeholder='white color, height of around 8-10 inches, made from high-quality materials and is perfect for use'></textarea>
              {showKeyboard && 
              <div className='text-sm' style={{color:'red'}}>Description Term must have at least 2 characters</div>
              }
               {showCharacter && 
              <div className='text-sm' style={{color:'red'}}>Description Term must have at most 250 characters</div>
              }
              </div>
...
<Link id='automate' onClick={handleMenuToggle} className='bg-white flex-row'>
        <div style={{marginLeft:'3ch'}} className='flex flex-row space-x-2 items-center'>
          ...
          <span id="colors" style={{color:'#cd1963'}} className='items-center text-2xl'> <MdOutlineProductionQuantityLimits/> </span>
          </div>
         ...
         <span>
          <p>AI tool generates Product Descriptions on user-defined parameters.</p>
         </span>
          </div> 
        </div>
</Link>
