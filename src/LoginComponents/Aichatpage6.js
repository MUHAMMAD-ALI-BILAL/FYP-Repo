...
export default function Aichatpage() {
  ...
  const Characters = "AK";
        ...
        async function handleSubmit(e){
          ...
          try{
            const response = await fetch("http://localhost:9001/youtubeTitle",{
              method:'POST',
              headers:{
  ...
  return (
    ...
    <div style={{}} className='flex flex-row max-h-screen'>
      ...
      <div>
              ...
              <label className='text-white'>Youtube Search</label>
              <input className='rounded-lg' style={{padding:'0.7ch'}} value={input} onChange={handleChange1} placeholder='Forza Horizon 5. OR Car Game.'></input>
              {showInput && 
              <div className='text-sm' style={{color:'red'}}>Search must have at least 2 characters</div>
              }
              </div>

              <div style={{width:'90%', marginLeft:'auto', marginRight:'auto'}} className='flex flex-col'>
              <label className='text-white'>Youtube Description</label>
              <textarea className='rounded-lg' style={{padding:'0.7ch'}} value={keyword} onChange={handleChange2} placeholder='1000 HP BMW M8 Forza Horizon 5 | Extreme car driving simulator | Car game | Racing in car 2 | Android game

                                    Cruising in a new BMW M8 Competition Coupe in Forza Horizon 5 using Thrustmaster steering wheel setup hope you enjoy the video!
                                    If you enjoyed this video and would like to see more driving please hit the LIKE and SUBSCRIBE button!

                                    PC Specification: (DESKTOP)
                                    Processor : Ryzen 5900x
                                    GPU : Nvidia RTX 3080ti
                                    RAM : 32GB DDR4'></textarea>
              {showKeyboard && 
              <div className='text-sm' style={{color:'red'}}>Description must have at least 2 characters</div>
              }
              </div>
...
<Link id='automate' onClick={handleMenuToggle} className='bg-white flex-row'>
        <div style={{marginLeft:'3ch'}} className='flex flex-row space-x-2 items-center'>
         ...
         <span>
          <p>AI tool generates Youtube Title on user-defined parameters.</p>
         </span>
          </div> 
        </div>
</Link>
