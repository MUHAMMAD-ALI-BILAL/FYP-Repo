...
export default function Aichatpage() {
  ...
  const Characters = "AK";
        ...
        async function handleSubmit(e){
          ...
          try{
            const response = await fetch("http://localhost:9001/email",{
              method:'POST',
              headers:{
...
<div id='scroller'>
      ...
      <div>
              ...
              <input className='rounded-lg' style={{padding:'0.7ch'}} value={input} onChange={handleChange1} placeholder='Brock'></input>
              {showInput && 
              <div className='text-sm' style={{color:'red'}}>Sender Name must have at least 2 characters</div>
              }
              </div>

              <div style={{width:'90%', marginLeft:'auto', marginRight:'auto'}} className='flex flex-col'>
              <label className='text-white'>Receiver Name</label>
              <input className='rounded-lg' style={{padding:'0.7ch'}} value={ReceiverName} onChange={handleChange4} placeholder='John'></input>
              {showInput && 
              <div className='text-sm' style={{color:'red'}}>Receiver Name must have at least 2 characters</div>
              }
              </div>

              <div style={{width:'90%', marginLeft:'auto', marginRight:'auto'}} className='flex flex-col'>
              <label className='text-white'>Purpose</label>
              <textarea className='rounded-lg' style={{padding:'0.7ch'}} value={keyword} onChange={handleChange2} placeholder='Request for Leave from job for 2 days. Note:"Alo write company name if required".'></textarea>
              {showKeyboard && 
              <div className='text-sm' style={{color:'red'}}>Search Term must have at least 2 characters</div>
              }
               {showCharacter && 
              <div className='text-sm' style={{color:'red'}}>Search Term must have at most 250 characters</div>
              }
              </div>

              <div style={{width:'90%', marginLeft:'auto', marginRight:'auto'}} className='flex flex-col'>
              <label className='text-white'>Key points</label>
              <textarea className='rounded-lg' style={{padding:'0.7ch'}} value={target} onChange={handleChange3} placeholder='Brother Wedding'></textarea>
              {/* {showKeyboard && 
              <div className='text-sm' style={{color:'red'}}>Search Term must have at least 2 characters</div>
              }
               {showCharacter && 
          ...
          <div>
            <div style={{width:'90%', marginLeft:'auto', marginRight:'auto'}} className='flex flex-row space-x-7'>
              ...
              <div style={{width:'100%'}}> 
                <div className='flex flex-col'>
                <label className='text-white'>Max Characters</label>
...
<Link id='automate' onClick={handleMenuToggle} className='bg-white flex-row'>
        <div style={{marginLeft:'3ch'}} className='flex flex-row space-x-2 items-center'>
          ...
          <span id="colors" style={{color:'red'}} className='items-center text-2xl'> <AiOutlineMail/> </span>
          </div>
         ...
         <span>
          <p>AI tool generates Email on user-defined parameters.</p>
         </span>
          </div> 
        </div>
</Link>
